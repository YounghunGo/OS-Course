...
struct sched_class* classpointer = NULL;//sched_my_class 주소 저장( 수정)
int CONFIG_MYSCHED;//0 : no lkm, 1 : in lkm
EXPORT_SYMBOL(CONFIG_MYSCHED);//수정
EXPORT_SYMBOL(classpointer);//수정
...
static inline void check_class_changed(struct rq *rq, struct task_struct *p,
				       ...
				       int oldprio)
{
	printk(KERN_DEBUG "check_class_changed call");
...
{
	//printk(KERN_DEBUG "sched_fork call");
	unsigned long flags;
	int cpu = get_cpu();
